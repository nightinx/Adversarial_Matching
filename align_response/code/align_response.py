...
def show_sent(sent,nlp):
    print(sent)
    doc = nlp(sent)
    for token in doc:
        print(token.text,token.pos_,token.dep_)

def write_as_json(hypo_list,theory_list,path,nlp):
    import random
    import jsonlines
    adversay_dict={}
...
def write_as_json_all(hypo_list,theory_list,path,nlp):
    adversay_dict={}
    for i,hypo1 in enumerate(hypo_list):
        adversay_dict[hypo1]=""
...
def main():
    ...
    write_as_json_all(hypo_list,theory_list,'./data/aligened_tree.jsonlines',nlp)
